...
function checkNestedStackParameterValues(values) {
	...
	if (values.ApiName.Ref) {
		// SAM uses the stackname for API name
		expect(values.ApiName).toEqual({
			Ref: "AWS::StackName"
		});
	} else {
		// SLS includes the API name in the stack
...
const defaultSuffixes = ["P90LatencyAlarm", "P95LatencyAlarm", "P99LatencyAlarm", "4xxAlarm", "5xxAlarm", "2xxAlarm"];

function checkLogicalIds(logicalIds, prefix, ...suffixes) {
	(suffixes || defaultSuffixes).map(x => prefix + x).forEach(logicalId => expect(logicalIds).toContain(logicalId));
}
